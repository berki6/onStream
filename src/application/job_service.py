...
def _owned_video_and_job(db: Session, video_id: str, user_id: int):
    ...
    if not job:
        raise AppError("Job not found", code=ErrorCode.JOB_NOT_FOUND)
    return video, job
...
def apply_action(db: Session, video_id: str, user_id: int, action: str):
    ...
    if action == "retry":
        if job.stage not in ("error", "cancelled", "ready"):
            raise AppError(
                "Job is still in progress", code=ErrorCode.JOB_CONFLICT
            )
        ...
        if not job_queue.enqueue_job(video_id, db):
            mark_enqueue_failed_and_raise(db, video_id)
        return job

    if action == "cancel":
        if job.stage in ("ready", "cancelled"):
            raise AppError(
                "Job cannot be cancelled", code=ErrorCode.JOB_CONFLICT
            )
    ...
    raise AppError(f"Unknown action: {action}", code=ErrorCode.JOB_BAD_REQUEST)
