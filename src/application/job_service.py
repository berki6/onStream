...
def apply_action(db: Session, video_id: str, user_id: int, action: str):
    ...
    if action == "retry":
        if job.stage not in ("error", "cancelled", "ready"):
            raise AppError(
                "Job is still in progress", code="conflict", status_code=409
            )
        ...
        db.commit()
        job_queue.enqueue_job(video_id, db)
        return job

    if action == "cancel":
        if job.stage in ("ready", "cancelled"):
            raise AppError(
                "Job cannot be cancelled", code="conflict", status_code=409
            )
    ...
    raise AppError(f"Unknown action: {action}", code="bad_request", status_code=400)
