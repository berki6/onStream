...
def _require_live_enabled() -> None:
    if not settings.LIVE_ENABLED:
        raise AppError("Live streaming is disabled", code=ErrorCode.LIVE_DISABLED)


def validate_stream_id(stream_id: str) -> str:
    if not stream_id or len(stream_id) != 12:
        raise AppError("Invalid stream ID format", code=ErrorCode.VALIDATION_INVALID_ID)
...
def _check_auth_secret(provided: Optional[str]) -> None:
    ...
    if not provided or provided != expected:
        raise AppError("Unauthorized", code=ErrorCode.LIVE_UNAUTHORIZED)
...
def resolve_master(stream: models.LiveStream) -> Path:
    ...
    raise AppError(
        "Live playlist not available", code=ErrorCode.LIVE_NOT_FOUND
    )
